...
def detectFaceOpenCVDnn(net, frame):
    ...
    函数 blobFromImage(InputArray image,
     ...
     预处理的图像frame；
     图片像素缩放为1.0；
     神经网络在训练的时候要求输入的图片尺寸300*300；
     对RGB图片的三个通道分别减去不同的值 104 117 123
     openCV中认为我们的图片通道顺序是BGR，但是我平均值假设的顺序是RGB，所以如果需要交换R和G，那么就要为true
     图像裁剪False.当值为True时，先按比例缩放，然后从中心裁剪成size尺寸
